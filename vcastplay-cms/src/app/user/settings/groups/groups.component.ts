...
export class GroupsComponent {
  ...
  onClickSave(event: Event, type: string) {
    this.confirmation.confirm({
      ...
      accept: () => {
        if (type === 'group') this.onSaveGroup();
        else this.onSaveSubGroup();
      }
    })
  }
  ...
  onClickRemoveGroup(event: Event, item: any, type: string) {
    this.confirmation.confirm({
      target: event.target as EventTarget,
      ...
      accept: () => {
        if (type === 'group') {
          this.onRemoveGroup(item);
        } else {
          this.onRemoveSubGroup(item);
        }
      }
    })
  }
  ...
  onPageChange(event: any, type: string) {
    ...
    if (type === 'group') {
      const { currentPage, itemsPerPage, ...meta } = this.paginatedGroup();
      this.paginatedGroup.set({ ...meta, currentPage: pageNumber, itemsPerPage: rows });
      this.groupService.onLoadGroups(pageNumber, event.rows);
    } else {
      const groupId = this.selectedGroup ? this.selectedGroup.id : 0;
      const { currentPage, itemsPerPage, ...meta } = this.paginatedSubGroup();
      this.paginatedSubGroup.set({ ...meta, currentPage: pageNumber, itemsPerPage: rows });
  ...
  onSaveGroup() {
    ...
    const mode = this.isEditGroup() ? 'update' : 'create';
    this.groupService.onSaveGroups(groupId, { name, description }, mode).subscribe({
      next: (res: any) => {
        if (mode === 'create') this.groups.set([...this.groups(), res]);
        else {
          const index = this.groups().findIndex((item: any) => item.id === groupId);
          if (index !== -1) this.groups()[index] = res;
        }
        this.message.add({severity:'success', summary: 'Success', detail: 'Group saved successfully!' });
      },
      error: (err: any) => this.message.add({severity:'error', summary: 'Error', detail: err.error.message }),
      complete: () => this.onClickCancel('groups')
    });
  }

  onSaveSubGroup() {
    if (this.subGroupData.invalid) {
      this.subGroupData.markAsDirty();
      this.message.add({severity:'error', summary: 'Error', detail: 'All fields are required!' });
      return;
    };
    const groupData: any = this.selectedGroup;
    const { id, name, description } = this.subGroupData.value;
    const mode = this.isEditSubGroup() ? 'update' : 'create';

    this.groupService.onSaveSubGroups(groupData.id, id, { name, description }, mode).subscribe({
      next: (res: any) => {
        if (mode === 'create') this.subGroups.set([...this.subGroups(), res]);
        else {
          const index = this.subGroups().findIndex((item: any) => item.id === id && item.groupId === groupData.id);
          if (index !== -1) this.subGroups()[index] = res;
        }
        this.message.add({severity:'success', summary: 'Success', detail: 'Sub Group saved successfully!' });
      },
      error: (err: any) => this.message.add({severity:'error', summary: 'Error', detail: err.error.message }),
      complete: () => this.onClickCancel('sub-group')
    });
  }

  onRemoveGroup(item: any) {
